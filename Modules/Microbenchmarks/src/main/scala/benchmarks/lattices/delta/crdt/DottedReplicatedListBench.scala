...
@Threads(1)
@State(Scope.Thread)
class DottedReplicatedListBench {

  @Param(Array("0", "1", "10", "100", "1000"))
  var listSize: Int = scala.compiletime.uninitialized

  type SUT = NamedDeltaBuffer[DottedReplicatedList[Int]]

  var rga: SUT        = scala.compiletime.uninitialized

  @Setup
  def setup(): Unit = {
    rga = NamedDeltaBuffer("a".asId, DottedReplicatedList.empty[Int]).mod(_.appendAll(0 until listSize)(using "".asId))
  }
