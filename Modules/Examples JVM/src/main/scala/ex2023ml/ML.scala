...
object MLTest {
  ...
  def multiply(i: SD): Signal[Double] = i.map(v => v * v)
  def app(i: SD): Signal[Double]      =
    multiply(dense(i))
  ...
  def diff_param(init: Double, bp: Signal[Boolean], `do`: SD): Signal[Double] =
    `do`.change.fold(init): (acc: Double, next: Diff[Double]) =>
       acc + (if bp.value then next.from.get else 0.0)

  def diff_dense(i: => SD, bp: => Signal[Boolean], `do`: => SD): (Signal[Double], Signal[Double]) =
  ...
  def diff_multiply(i: => SD, bp: => Signal[Boolean], `do`: => SD): (Signal[Double], Signal[Double]) =
  ...
  def diff_app(i: SD, bp: Signal[Boolean], gt: SD, learn_rate: SD): Signal[Double] =
     lazy val (
       (x: SD, di: SD),
       (o: SD, dx: SD),
       (d_o: SD)
