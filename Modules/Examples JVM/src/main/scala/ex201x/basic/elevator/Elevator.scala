package ex201x.basic.elevator

import ex201x.basic.datastructures.SQueue
...
class Elevator(val nFloors: Int) {
  ...
  val destination: Signal[Int] = Signal {
    queue.head.value match {
      case None         => position.value
      case Some(target) => FloorPos(target)
    }
  }
  val distance: Signal[Int]  = Signal { destination.value - position.value }
  val direction: Signal[Int] = Signal { math.signum(distance.value) }

  val stopped: Signal[Boolean] = Signal { speed.value == 0 }
  ...
  val currentFloor: Signal[Int] = Signal {
  ...
  val waitingTime: Signal[Int] = Fold(0)(
    reachedFloor branch { _ => WaitingTime },
