...
import ex2026lofi_acl.legacy.DeltaMapWithPrefix
import ex2026lofi_acl.legacy.bft.{PartialDelta, PartialReplicationPeerSubsetSolver}
import ex2026lofi_acl.legacy.monotonic.MonotonicAclSyncMessage.*
import ex2026lofi_acl.legacy.monotonic.Operation.WRITE
import ex2026lofi_acl.sync.{MessageReceiver, Replica}
        ...
        case RequestMissing(remoteRdtDots, remoteAclDots) =>
          ...
          val _                =
            connectionManager.sendMultiple(
              sender,
              missingAclDeltas.toArray*
            ) // No need to lock or filter for ACL deltas

          val missingRdtDeltas = rdtDots.subtract(remoteRdtDots)
          val deltas           =
            rdtDeltas.retrieveDeltas(missingRdtDeltas).map[Delta[RDT]]((dot, delta) =>
              Delta(delta, dot, aclDots)
            ).toArray
