  ...
  def baselineBufferKRList(blackhole: Blackhole, state: EvalORMapState, resultCapture: ResultCapture): Unit = {
    ...
    blackhole.consume(state.randomArr.foreach { item =>
      if math.abs(item % 10) < state.localReplicaShare then {
        ...
        val delta = foreignReplica.produceDelta(a =>
          Eval.performKRListOperation(a, item % state.mapSize, foreignReplica.replicaId)
        )
  ...
  def baselineBufferRecipeBook(blackhole: Blackhole, state: EvalORMapState, resultCapture: ResultCapture): Unit = {
    ...
    blackhole.consume(state.randomArr.foreach { item =>
      if math.abs(item % 10) < state.localReplicaShare then {
        ...
        val delta = foreignReplica.produceDelta(a =>
          Eval.performRecipeBookOperation(a, item % state.mapSize, foreignReplica.replicaId)
        )
        localReplica.applyDelta(delta)
      }
      resultCapture.recordBufferSize(localReplica.buffer.getSize)
...
object EvalORMap {
  ...
  def performORMapOperationEWFlag(
    ...
    if orMap.entries.isEmpty then
        return orMap.update(
          random,
          if random % 2 == 0 then EnableWinsFlag.empty.enable(using localUid)() else EnableWinsFlag.empty.disable()
        )(using localUid)
    math.abs(random % 2) match {
      ...
      case _ => orMap.transform(random) {
          ...
          case None        => Option(if random % 2 == 0 then EnableWinsFlag.empty.enable(using localUid)()
            else EnableWinsFlag.empty.disable())
        }(using localUid)
    }
  }
