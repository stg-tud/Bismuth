...
class HyParViewIO[State](
  ...
  private val connections               = mutable.LinkedHashMap.empty[Uid, Connection]
  private val connectionToPeer          = mutable.LinkedHashMap.empty[Connection, Uid]
  ...
  private def rememberPeerIdentity(peer: Uid, conn: Connection): Unit =
  ...
  private def cleanupInactiveConnection(peer: Uid, conn: Connection, reason: String): Unit = {
  ...
  private def rememberConnection(peer: Uid, conn: Connection): Unit = {
    ...
    checkInvariant(
      !membership.activeView.contains(peer) || plumtree.peerRoles.contains(Peer(peer)) || connecting.contains(peer),
      s"remembered active connection without plumtree peer=${Uid.unwrap(peer)}"
    )
    ()
  }
