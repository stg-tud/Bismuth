...
class WebSocketConnectionDetailsResolver[T: JsonValueCodec] extends ChannelResolver[T] {
  ...
  override def connect(details: ChannelConnectDescriptor, label: String): Option[LatentConnection[T]] =
    details match
      case ChannelConnectDescriptor.WebSocket(url) => Some(jsonConnection(WebsocketConnect.connect(url), s"websocket-json:$label"))
      case ChannelConnectDescriptor.Tcp(host, port) => Some(jsonConnection(WebsocketConnect.connect(s"ws://$host:$port"), s"websocket-json:$label"))
      case _                                 => None
}
