  ...
  def getContents(): Div = {
    ...
    val noAppointmentsCrossing = (
      ...
      Invariant {
        ...
        cal1.state.elements.forall { a =>
          cal2.state.elements.forall { b =>
            if a != b then {
    ...
    val addAppointment = Interaction[Calendar, Appointment]
      ...
      .requires { (cal: Calendar, a) => !cal.state.contains(a) }
      .executes { (cal: Calendar, a) => cal.clearDeltas().mod(_.add(a)) }
      .ensures { (cal: Calendar, a) => cal.state.contains(a) }
    ...
    def events[T](cal: Calendar)(mapper: Appointment => Event[T]): Event[T] = {
      val events = cal.state.elements.map(mapper)
    ...
    val workCalendarRDT: Signal[Calendar] =
      Storing.storedAs(storagePrefix + "-work", DeltaBuffer(ReplicatedSet.empty[Appointment])) { init =>
        Fold(init)(
    ...
    val vacationCalendarRDT: Signal[Calendar] =
      Storing.storedAs(storagePrefix + "-vacation", DeltaBuffer(ReplicatedSet.empty[Appointment])) { init =>
        Fold(init)(
    ...
    val workCalList: Signal[List[Appointment]] = workCalendarRDT.map { it => it.state.elements.toList.sortBy(_.start) }
    ...
    val vacationCalList: Signal[List[Appointment]] = vacationCalendarRDT.map { it =>
      it.state.elements.toList.sortBy(_.start)
    }
