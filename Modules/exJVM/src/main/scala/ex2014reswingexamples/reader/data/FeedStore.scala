package ex2014reswingexamples.reader.data
...
class FeedStore(
  ...
  val channels: Signal[Map[RSSChannel, Signal[Set[RSSItem]]]] =
    addChannel.fold(Map.empty[RSSChannel, Signal[Set[RSSItem]]]) { (map, channel) => // #SIG //#IF
      map + (channel ->
