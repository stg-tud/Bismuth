package ex2014reswingexamples.reshapes.drawing
...
class DeleteShape(shapeToDelete: Shape) extends Command {
  override def execute(shapes: List[Shape]): List[Shape] =
    shapes filterNot (_ == shapeToDelete)

  override def revert(shapes: List[Shape]): List[Shape] =
    shapeToDelete :: shapes

  override def description: String = "Delete %s".format(shapeToDelete)
}
...
class CreateShape(shapeToCreate: Shape) extends Command {
  ...
  override def description: String = "Create %s".format(shapeToCreate)
}
...
class EditShape(shapeBeforeEdit: Shape, shapeAfterEdit: Shape) extends Command {
  ...
  override def description: String = "Edit %s".format(shapeAfterEdit)
}
...
class MergeDrawingSpaces(eventToMerge: DrawingSpaceState) extends Command {
  ...
  override def description: String = "Merge with %s".format(eventTitle.now)
}
