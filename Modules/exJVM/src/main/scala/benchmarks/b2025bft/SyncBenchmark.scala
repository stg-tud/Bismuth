...
class SyncBenchmark {
  ...
  def sync(): Unit = {
    ...
    for fp <- lst do
        try {
          ...
          case _ =>
            MyCollector.add(b2025bft.Measurement("BLOOM_FAILED"))
        }
  ...
  def tearDown(): Unit = {
    ...
    b2025bft.Measurement.writeCSVRows(writer, allValues)
  }
  ...
  private object MyCollector {
    private val buf = ListBuffer[b2025bft.Measurement]()

    def add(measurement: b2025bft.Measurement): Unit = synchronized { buf += measurement }: Unit

    def getAll: Seq[b2025bft.Measurement] = buf.toList
