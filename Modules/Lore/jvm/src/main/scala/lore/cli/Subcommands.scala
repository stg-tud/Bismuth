...
object Parse {
  ...
  val parseSourceCode: Opts[Parse] =
    Opts.subcommand("parse", help = s"Parse a $languageName program and print the AST.") {
      Options.inputOpts
    }.map(Parse.apply)
...
object ToViper {
  ...
  val toViper: Opts[ToViper] =
    Opts.subcommand("viperize", help = s"Compile a $languageName program to Viper IL for verification.") {
