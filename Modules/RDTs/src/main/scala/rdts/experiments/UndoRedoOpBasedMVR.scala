...
object UndoRedoOpBased {
  ...
  case class MVR[T](
    ...
    def set(value: T): (MVR[T], Operation[T]) = {
      applyLocalOperation(OperationType.set(value))
    }

    def delete(): (MVR[T], Operation[T]) = {
      applyLocalOperation(OperationType.delete)
    }

    def undo(): (MVR[T], Option[Operation[T]]) = {
    ...
    def redo(): (MVR[T], Option[Operation[T]]) = {
    ...
    private def applyLocalOperation(operationType: OperationType[T]): (MVR[T], Operation[T]) =
    ...
    def applyRemoteOperation(operation: Operation[T]): MVR[T] = {
    ...
    private def apply(operation: Operation[T]): MVR[T] = {
  ...
  object MVR {
    def forReplica[T](replicaId: Uid): MVR[T] = MVR(
    ...
    def createReplicas[T](n: Int): Array[MVR[T]] = {
      Array.tabulate(n) { i =>
        MVR.forReplica[T](Uid(s"R${i + 1}"))
      }
    }
  }
