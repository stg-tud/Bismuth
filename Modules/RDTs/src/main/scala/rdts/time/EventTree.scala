...
sealed trait EventTree:
    ...
    def increment(id: IdTree): EventTree = {
      ...
      val normalizedEventTree = this.normalize

      val filledEventTree = normalizedEventTree.fill(normalizedId)

      if !(normalizedEventTree eq filledEventTree) then {
        filledEventTree
      } else {
    ...
    private[time] def fill(id: IdTree): EventTree = (id, this) match
        ...
        case (IdTree.Branch(IdTree.Leaf(1), ir), e @ EventTree.Branch(n, el, er)) =>
          ...
          if (er eq erFilled) && (el == elFilled) then {
            e // No change
          } else {
            Branch(
              n,
        ...
        case (IdTree.Branch(il, IdTree.Leaf(1)), e @ EventTree.Branch(n, el, er)) =>
          ...
          if (el eq elFilled) && (er == erFilled) then {
            e // No change
          } else {
            Branch(
              n,
        ...
        case (IdTree.Branch(il, ir), e @ EventTree.Branch(n, el, er)) =>
          val elFilled = el.fill(il)
          val erFilled = er.fill(ir)
          if (el eq elFilled) && (er eq erFilled) then {
            e // No change
          } else {
            Branch(
              n,
    ...
    private[time] def grow(id: IdTree): (EventTree, Int) = (id, this) match
        ...
        case (IdTree.Leaf(1), ev) =>
          ev.normalize.fill(IdTree.Leaf(1)).grow(IdTree.Leaf(1))
  ...
  given NormalForm[EventTree] with
      extension (tree: EventTree)
          def normalize: EventTree = tree match {
            ...
            case Branch(n, l, r) =>
              val (lNorm, rNorm) = (l.normalize, r.normalize)
              ...
              if min == 0 then {                       // Already normalized
                if (lNorm eq l) && (rNorm eq r) then { // Reuse reference if tree is already normalized
                  tree
                } else {
