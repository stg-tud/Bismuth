...
import rdts.protocols.paper.{MultipaxosPhase, PipePaxos}
import rdts.protocols.Participants

class PipePaxosTest extends munit.FunSuite {
  ...
  val emptyPaxosObject: PipePaxos[Int] = PipePaxos.empty
  test("happy path") {
    ...
    assertEquals(testPaxosObject.readDecisionsSince(0).toList, List.empty)
    assertEquals(testPaxosObject.phase, MultipaxosPhase.Voting)
    testPaxosObject = testPaxosObject.merge(testPaxosObject.upkeep(using id3))
    assertEquals(testPaxosObject.readDecisionsSince(0).toList, List(1))
    ...
    assertEquals(testPaxosObject.readDecisionsSince(0).toList, List(1))
    testPaxosObject = testPaxosObject.merge(testPaxosObject.upkeep(using id2))
    assertEquals(testPaxosObject.readDecisionsSince(0).toList.sorted, List(1, 12))

    // replica 3 starts new leader election
    testPaxosObject = testPaxosObject.merge(testPaxosObject.startLeaderElection(using id3))
    assertEquals(testPaxosObject.readDecisionsSince(0).toList.sorted, List(1, 12), "log survives new leader election")
  ...
  test("conflicting proposals") {
    ...
    assert(testPaxosObject.readDecisionsSince(0).head == 1 || testPaxosObject.readDecisionsSince(0).head == 2)
  }
