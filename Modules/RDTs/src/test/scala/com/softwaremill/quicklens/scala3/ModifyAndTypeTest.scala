...
class ModifyAndTypeTest extends munit.FunSuite {
  test("modify an & type object") {
    val ab: A & B = A(0)

    val modified = ab.modify(_.a).setTo(1)

    assertEquals(modified.a, 1)
  }

  test("modify an & type object 1") {
    val ab: B & A = A(0)
