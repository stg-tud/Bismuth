...
A lattice for a type :m{A} is a single operation:
...
In Scala 3 a :m{Lattice[A]} is provided as a :b{given} instance.
You bring the merge syntax into scope with an import:
