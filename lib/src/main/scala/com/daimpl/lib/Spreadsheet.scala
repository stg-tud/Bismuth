...
case class Spreadsheet(
  ...
  def addRow()(using LocalUid)(using context: Dots): D = {
    ...
    val keepRowsUpdate = keepRows.update(
      rowId,
      MultiValueRegister(Map(VectorClock(Map(replicaId -> CausalTime.now().time)) -> operationId))
    )
    Obrem(
      Spreadsheet(
  ...
  def editCell(visibleRowIdx: Int, visibleColIdx: Int, cellContent: String | Null)(using
    ...
    val keepRowsUpdate = keepRows.update(
      rowId,
      MultiValueRegister(Map(VectorClock(Map(replicaId -> CausalTime.now().time)) -> rowOperationId))
    )
    ...
    val keepColsUpdate = keepCols.update(
      colId,
      MultiValueRegister(Map(VectorClock(Map(replicaId -> CausalTime.now().time)) -> colOperationId))
    )
    ...
    Obrem(
      ...
      contentUpdate.observed `union` keepRowsUpdate.observed `union` keepColsUpdate.observed `union` Dots.from(
        Array(rowId, colId /*, rowOperationId, colOperationId*/ )
      ),
